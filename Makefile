...
PYTEST_COVERAGE_OPTIONS = --cov=$(LIBRARY_DIRS) --cov-report term-missing
...
check-py-black: ## Runs black in check mode (no changes)
	$(RUN_PYPKG_BIN) black --check --line-length 80 --fast .
...
format-py: ## Runs black, makes changes where necessary
	$(RUN_PYPKG_BIN) black --line-length 80 .

.PHONY: format
format: sort-py format-py
